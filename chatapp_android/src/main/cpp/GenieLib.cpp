...
extern "C" JNIEXPORT void JNICALL Java_com_quicinc_chatapp_GenieWrapper_setToolDefinitions(JNIEnv* env,
                                                                                            jobject /* this */,
                                                                                            jlong genie_wrapper_handle,
                                                                                            jstring tool_definitions)
{
    try
    {
        std::string tools = env->GetStringUTFChars(tool_definitions, 0);
        App::GenieWrapper* genie_wrapper = reinterpret_cast<App::GenieWrapper*>(genie_wrapper_handle);
        genie_wrapper->SetToolDefinitions(tools);
    }
    catch (std::exception& e)
    {
        jclass exception_cls = env->FindClass("java/lang/RuntimeException");
...
extern "C" JNIEXPORT void JNICALL Java_com_quicinc_chatapp_GenieWrapper_submitToolResponse(JNIEnv* env,
                                                                                            jobject /* this */,
                                                                                            jlong genie_wrapper_handle,
                                                                                            jstring tool_result,
                                                                                            jobject callback)
{
    try
    {
        ...
        std::string result = env->GetStringUTFChars(tool_result, 0);

        App::GenieWrapper* genie_wrapper = reinterpret_cast<App::GenieWrapper*>(genie_wrapper_handle);
        genie_wrapper->SubmitToolResponse(result, env, callback, onNewStringMethod);
    }
    catch (std::exception& e)
    {
        jclass exception_cls = env->FindClass("java/lang/RuntimeException");
