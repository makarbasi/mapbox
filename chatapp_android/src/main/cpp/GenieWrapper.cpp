...
void GenieWrapper::SetToolDefinitions(const std::string& tools)
{
    prompt_handler.SetToolDefinitions(tools);
}

std::string GenieWrapper::SubmitToolResponse(const std::string& tool_result,
                                              JNIEnv* env,
                                              jobject callback,
                                              jmethodID onNewStringMethod)
{
    ...
    std::string tagged_prompt = prompt_handler.GetToolResponsePrompt(tool_result);
    if (GENIE_STATUS_SUCCESS != GenieDialog_query(m_dialog_handle, tagged_prompt.c_str(),
                                                  GenieDialog_SentenceCode_t::GENIE_DIALOG_SENTENCE_COMPLETE,
                                                  GenieCallBack, &user_data))
    {
        __android_log_print(ANDROID_LOG_ERROR, "ChatApp", "Failed to get response for tool result.");
    }
