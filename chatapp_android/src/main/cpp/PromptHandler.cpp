...
PromptHandler::PromptHandler(const std::string& models_path)
{
    ...
    try
    {
        ...
        m_ipython_prefix = "<|start_header_id|>ipython<|end_header_id|>\n\n";
        m_ipython_suffix = "<|eot_id|>";
        if (tmpl.contains("ipython_prefix"))
            m_ipython_prefix = tmpl["ipython_prefix"].get<std::string>();
        if (tmpl.contains("ipython_suffix"))
            m_ipython_suffix = tmpl["ipython_suffix"].get<std::string>();
    }
...
std::string PromptHandler::GetPromptWithTag(const std::string& user_prompt)
{
    ...
    if (m_is_first_prompt)
    {
        ...
        if (!m_tool_definitions.empty())
        {
            prompt += "\n\n";
            prompt += m_tool_definitions;
        }
...
void PromptHandler::SetToolDefinitions(const std::string& tools)
{
    m_tool_definitions = tools;
}

std::string PromptHandler::GetToolResponsePrompt(const std::string& tool_result)
{
    std::string prompt;
    prompt += m_ipython_prefix;
    prompt += tool_result;
    prompt += m_ipython_suffix;
    prompt += m_assistant_prefix;
    return prompt;
}
